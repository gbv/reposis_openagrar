  ...
  <xsl:template match="mods:name">
    <xsl:if test="mods:displayForm or @valueURI">
      <creator>
        ...
        <xsl:if
        <nameIdentifier nameIdentifierScheme="GND" schemeURI="http://d-nb.info/gnd/1059149273">1059149273</nameIdentifier>
        
        <mods:nameIdentifier type="gnd">1059149273</mods:nameIdentifier>
      </creator>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="mods:nameIdentifier">
    <xsl:choose>
      <xsl:when test="@type='gnd'">
        <nameIdentifier nameIdentifierScheme="GND" schemeURI="http://d-nb.info/gnd/1059149273">
          <xsl:value-of select="."/>
        </nameIdentifier>
      </xsl:when>
    </xsl:choose>
