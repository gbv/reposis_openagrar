...
<xsl:stylesheet version="1.0"
        ...
        <xsl:variable name="nameType">
          <xsl:choose>
            ...
            <xsl:when test="@type='corporate'">
              <xsl:value-of select="'Organizational'"/>
            </xsl:when>
          </xsl:choose>
        </xsl:variable>
  ...
  <xsl:template match="mods:nameIdentifier">
    <xsl:choose>
      <xsl:when test="@type='gnd'">
        <nameIdentifier nameIdentifierScheme="GND">
          <xsl:value-of select="."/>
        </nameIdentifier>
      </xsl:when>
    </xsl:choose>
      ...
      <xsl:choose>
        ...
        <xsl:when test="mods:name[mods:role/mods:roleTerm/text()='his' and @valueURI]">
          ...
          <xsl:choose>
            ...
            <xsl:otherwise>
              <xsl:value-of select="concat('otw1',$MCR.DOI.HostingInstitution)" />
            </xsl:otherwise>
          </xsl:choose>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="concat('otw2',$MCR.DOI.HostingInstitution)" />
        </xsl:otherwise>
      </xsl:choose>
    </publisher>
  </xsl:template>
  ...
  <xsl:template name="dates">
    <dates>
      <xsl:choose>
        <xsl:when test="mods:originInfo[@eventType='creation' or @eventType='publication']">
        ...
        <xsl:otherwise>
          <xsl:apply-templates select="mods:originInfo[not(@eventType)]/*"/>
        </xsl:otherwise>
      </xsl:choose>
    </dates>
  </xsl:template>
