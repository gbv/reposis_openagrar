...
>
  ...
  <xsl:template name="mods2schemaOrg">
	...
	Take 1. mods:titleInfo as headline and all further mods:titleInfo as 
      ...
      <!-- identifiers -->
      <!-- Add registryID and url - OA-380#116 -->
        ...
        <fn:array key="identifier">
          <xsl:for-each select="mods:identifier">
            ...
            <fn:map>
              ...
              <xsl:if test="@typeURI != '#'">
                <fn:string key="url">		     
			      <xsl:value-of select="concat(@typeURI, text())" />
                </fn:string>
              </xsl:if>
            </fn:map>
          </xsl:for-each>
      ...
      <xsl:if test="mods:originInfo[@eventType='publication']/mods:publisher">
        <fn:map key="publisher">
          <fn:string key="@type">Organization</fn:string>
          <fn:string key="name">
      ...
      <!-- Book Stuff-->
      <!-- OA-380#116 show ISBN like all identifier -->
      ...
      <xsl:if test="count(mods:subject[@xlink:type='simple']/mods:topic) &gt; 0">
		<fn:array key="keywords">
          <xsl:for-each select="mods:subject[@xlink:type='simple']/mods:topic">
            <fn:string>
			  <xsl:value-of select="." />
			</fn:string>
		  </xsl:for-each>
